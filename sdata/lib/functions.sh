...
function v(){
  ...
  if $ask;then
    while true;do
      ...
      case $p in
        [yY])
          echo -e "${STY_BLUE}OK, executing...${STY_RST}"
          break
          ;;
        [eE])
          echo -e "${STY_BLUE}Exiting...${STY_RST}"
          exit
          break
          ;;
        [sS])
          echo -e "${STY_BLUE}Alright, skipping this one...${STY_RST}"
          execute=false
          break
          ;;
        "yesforall")
          echo -e "${STY_BLUE}Alright, won't ask again. Executing...${STY_RST}"
          ask=false
          break
          ;;
        *)
...
function x(){
  ...
  while [ $cmdstatus == 1 ] ;do
    ...
    case $p in
      [iI])
        echo -e "${STY_BLUE}Alright, ignore and continue...${STY_RST}"
        cmdstatus=2
        ;;
      [eE])
        echo -e "${STY_BLUE}Alright, will exit.${STY_RST}"
        break
        ;;
      *)
  ...
  case $cmdstatus in
    0)
      echo -e "${STY_BLUE}[$0]: Command \"${STY_GREEN}$*${STY_BLUE}\" finished.${STY_RST}"
      ;;
    1)
      ...
      ;;
    2)
      echo -e "${STY_RED}[$0]: Command \"${STY_GREEN}$*${STY_RED}\" has failed but ignored by user.${STY_RST}"
      ;;
  esac
}
...
function prevent_sudo_or_root(){
  case $(whoami) in
    root)
      echo -e "${STY_RED}[$0]: This script is NOT to be executed with sudo or as root. Aborting...${STY_RST}"
      exit 1
      ;;
  esac
}
...
function validate_path_in_directory() {
  ...
  local abs_file=$(cd "$(dirname "$file_path")" 2>/dev/null && pwd -P)/$(basename "$file_path") || return 1
  local abs_dir=$(cd "$allowed_dir" 2>/dev/null && pwd -P) || return 1
