...
function ask_backup_configs(){
  ...
  while true;do
    ...
    case $p in
      [yY])
        echo -e "${STY_BLUE}OK, doing backup...${STY_RST}"
        local backup=true
        break
        ;;
      [nN])
        echo -e "${STY_BLUE}Alright, skipping...${STY_RST}"
        local backup=false
        break
        ;;
      *)
  ...
  *)
    for i in $(find dots/.config/ -mindepth 1 -maxdepth 1 ! -name 'quickshell' ! -name 'fish' ! -name 'hypr' -exec basename {} \;); do
      ...
      if [ -d "dots/.config/$i" ]; then
        warning_rsync
        v rsync -av --delete "dots/.config/$i/" "$XDG_CONFIG_HOME/$i/"
      elif [ -f "dots/.config/$i" ]; then
        warning_rsync; v rsync -av "dots/.config/$i" "$XDG_CONFIG_HOME/$i"
      fi
    done
    ;;
esac
