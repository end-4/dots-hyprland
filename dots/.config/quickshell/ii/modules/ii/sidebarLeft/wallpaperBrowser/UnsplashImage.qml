...
Button {
    ...
    property string fileName: `unsplash-${imageData.id}.jpg`
                        ...
                        MenuButton {  
                            ...
                            onClicked: {  
                                ...
                                Quickshell.execDetached(["bash", "-c",   
                                    `mkdir -p '${root.downloadPath}' && curl '${root.imageData.file_url}' -o '${root.downloadPath}/${root.fileName}' && notify-send '${Translation.tr("Download complete")}' '${root.downloadPath}/${root.fileName}' -a 'Shell'`  
                                ])  
                            }  
                        }  
                        MenuButton {  
                            ...
                            onClicked: {  
                                root.showActions = false;  
                                const fullPath = `${root.downloadPath}/${root.fileName}`;  
                                Quickshell.execDetached(["bash", "-c",   
                                    `mkdir -p '${root.downloadPath}' && curl '${root.imageData.file_url}' -o '${fullPath}' && ${Directories.wallpaperSwitchScriptPath} --image '${fullPath}' && notify-send '${Translation.tr("Wallpaper applied")}' '${root.fileName}' -a 'Shell'`  
                                ])  
                            }  
                        }
                    }
