...
StyledOverlayWidget {
    ...
    contentItem: RowLayout {
        ...
        property string basePath: Quickshell.shellPath("assets/icons/catwalk/")
        ...
        Timer {
            ...
            onTriggered: {
                if (display.cpuUsage < display.idleThreshold) {
                    if (display.sourceIndex >= display.idleFrameCount)
                        display.sourceIndex = 0;

                    display.currentIcon = display.basePath + "my-idle-" + display.sourceIndex + "-symbolic.svg";
                } else {
                    if (display.sourceIndex >= display.activeFrameCount)
                        display.sourceIndex = 0;
