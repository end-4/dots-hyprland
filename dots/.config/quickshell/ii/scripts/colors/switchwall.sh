...
VIDEO_OPTS="no-audio loop hwdec=auto hwdec-codecs=all scale=bilinear interpolation=no video-sync=display-resample panscan=1.0 video-scale-x=1.0 video-scale-y=1.0 video-align-x=0.5 video-align-y=0.5 load-scripts=no vd-lavc-threads=2 cache=no demuxer-max-bytes=10M ontop-level=desktop gpu-api=vulkan fps=30"
