...
VIDEO_OPTS="no-audio loop hwdec=vaapi hwdec-codecs=all vo=gpu gpu-api=vulkan scale=bilinear interpolation=no video-sync=display-resample panscan=1.0 video-scale-x=1.0 video-scale-y=1.0 video-align-x=0.5 video-align-y=0.5 load-scripts=no cache=no demuxer-max-bytes=10M vd-lavc-threads=2 fps=30"
