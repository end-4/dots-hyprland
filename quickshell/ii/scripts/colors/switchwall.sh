...
kill_existing_mpvpaper() {
    # Отправляем сигнал на завершение всем процессам mpvpaper
    pkill -f -9 mpvpaper || true
    # Ждем, пока все процессы mpvpaper не будут фактически завершены
    while pgrep -f mpvpaper > /dev/null; do
        sleep 0.1
    done
}
...
create_restore_script() {
    local video_path=$1
    local log_file="$HOME/wallpaper_restore.log"
