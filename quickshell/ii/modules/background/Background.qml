...
Variants {
    ...
    PanelWindow {
        ...
        // НОВЫЙ БЛОК ДЛЯ ЗАПУСКА ВИДЕООБОЕВ
        Component.onCompleted: {
            if (bgRoot.wallpaperIsVideo) {
                const restoreScriptPath = `${FileUtils.xdgConfigPath}/hypr/custom/scripts/__restore_video_wallpaper.sh`;
                Quickshell.execDetached(["bash", "-c", restoreScriptPath]);
            }
        }
