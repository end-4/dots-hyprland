...
**URL**: https://dots-hyprland.vercel.app
...
**Agent rule**: After making any code change:
1. Update the relevant guide page in `docs/src/content/docs/guide/`
2. Check `docs/src/content/docs/ai-agents/changelog/` for the matching changelog entry
3. Fill in the `## AI Agent Notes` section with context on WHY the change was made, any gotchas, and what other agents should know

**Changelog entries** are auto-generated by `.github/workflows/docs-changelog.yml` on every push to `main`. They appear in `docs/src/content/docs/ai-agents/changelog/YYYY-MM-DD-<short-sha>.md`.

**Deployment**: Automatic via `.github/workflows/docs-deploy.yml` using Vercel. Requires secrets: `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`.
