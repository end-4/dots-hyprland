...
string getIconPath(string iconname) {
    ...
    string path = readIfExists("/home/" + username + "/.cache/ags/search/" + iconname);
    if (path == "") {
        path = exec(string("geticons -t " + iconTheme + " " + string(iconname) +
                           " | head -n 1")
                        .c_str());
        // cout << "path: " << path << '\n';
        writeToFile("/home/" + username + "/.cache/ags/search/" + iconname, path);
...
int main(int argc, char* argv[]) {
    ...
    // string fileToWrite = "/home/" + username + "/.cache/ags/search/entries.txt";
    // freopen(fileToWrite.c_str(), "w", stdout);
