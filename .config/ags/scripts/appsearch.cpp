...
void calcPrompt() {
    cout << "[{\"name\":\"Calculator - Type "
            "something!\",\"icon\":\"assets/svg/dark/"
...
void getAppNames() {
    string searchCommand =
        "cat '/home/" + username +
        "/.cache/ags/search/entrynames.txt' | fzf --filter=\"" + searchTerm +
        "\" | head -n 10";
...
void getAppJson() {
    ifstream file("/home/" + username + "/.cache/ags/search/entries.txt");
    file >> appEntries;
}
...
void tryCalculate() {
    ...
    cout
        << "[{\"name\":\"" << results
        << "\",\"icon\":\"assets/svg/dark/calculator.svg\",\"exec\":\"wl-copy '"
        << results << "'\"}]" << endl;
    // if (updateInfo) {
    //     string updateCmd =
    //         "eww update "
...
int main(int argc, char* argv[]) {
    exec("~/.config/ags/scripts/allappnames > ~/.cache/ags/search/entrynames.txt");
    exec("~/.config/ags/scripts/allapps > ~/.cache/ags/search/entries.txt");
