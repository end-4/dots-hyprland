...
const time = Variable('', {
	poll: [userOptions.time.interval, () => GLib.DateTime.new_now_local().format(userOptions.time.format)],
});

const date = Variable('', {
	poll: [userOptions.time.dateInterval, () => GLib.DateTime.new_now_local().format(userOptions.time.dateFormatLong)],
});
	...
	Box({
		...
		children: [
			...
			UtilButton({
				...
				icon: 'wallpaper',
				onClicked: () => {
					Utils.execAsync(`${App.configDir}/scripts/getRandomWallpaper.sh`).catch(print);
				},
			}),
		],
	});
...
const BatteryModule = () =>
	Stack({
		...
		children: {
			laptop: Box({
				className: 'spacing-h-4',
				children: [BarGroup({ child: Utilities() }), BarGroup({ child: BarBattery() }), BarGroup({ child: Weather() })],
			}),
			desktop: BarGroup({
				child: Box({
					hexpand: true,
