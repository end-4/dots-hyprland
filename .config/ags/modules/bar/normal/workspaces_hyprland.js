...
const WorkspaceContents = (count = 10) => {
    return DrawingArea({
        ...
        attribute: {
            ...
            workspaceGroup: 0,
            fontDesc: null,
            initFontDesc: function(self) {
                if (!self.attribute.fontDesc) {
                    ...
                    const fontFamily = workspaceFontFamily.length > 0 ? 
                        `"${workspaceFontFamily[0]}"` : 
                    
                    self.attribute.fontDesc = Pango.font_description_from_string(
                        `${fontFamily} ${getFontWeightName(workspaceFontWeight)} ${workspaceFontSize}`
                    );
                }
                return self.attribute.fontDesc;
            },
            updateMask: (self) => {
        ...
        setup: (area) => area
            ...
            .on('draw', Lang.bind(area, (area, cr) => {
                ...
                const fontDesc = area.attribute.initFontDesc(area);
                ...
                for (let i = 1; i <= count; i++) {
                    ...
                    const wsLabel = userOptions.workspaces.labels?.[i + offset] ?? `${i + offset}`;
