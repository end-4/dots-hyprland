...
export default function WindowTitle({ gdkmonitor, monitorId }: { gdkmonitor: Gdk.Monitor; monitorId: number }) {
    ...
    const focused = Variable.derive([focusedClient, focusedWorkspace], (client, workspace) => {
        return { client, workspace };
    });
    const scrollPos = () => Gtk.Adjustment.new(6, 0, 100, 1, 10, 0);

    return (
        <box className="bar-space-button" vertical={true} onDestroy={focused.drop}>
            <scrollable hexpand={true} vexpand={true} hscroll={Gtk.PolicyType.AUTOMATIC} vscroll={Gtk.PolicyType.NEVER}>
                {bind(focused).as((f) => {
                    const topdesc =
                        f.client == null ? 'Desktop' : bind(f.client, 'class').as((_class) => _class ?? 'Desktop');
                    const title =
                        f.client == null
                            ? `Workspace ${f.workspace?.id ?? ''}`
                            : bind(f.client, 'title').as((title) => title ?? `Workspace ${f.workspace?.id ?? ''}`);
