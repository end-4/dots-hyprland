...
const timeBefore = (time1: number[], time2: number[]) => { // Arrays of [hour, minute]
    if (time1[0] == time2[0]) return time1[1] < time2[1];
    return time1[0] < time2[0];
}

const timeSame = (time1: number[], time2: number[]) => // Arrays of [hour, minute]
    (time1[0] == time2[0] && time1[1] == time2[1]);

const timeBeforeOrSame = (time1: number[], time2: number[]) => // Arrays of [hour, minute]
    (timeBefore(time1, time2) || timeSame(time1, time2));

const timeInRange = (time: number[], rangeStart: number[], rangeEnd: number[]) => { // Arrays of [hour, minute]
