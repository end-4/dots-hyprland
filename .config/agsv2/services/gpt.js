...
class GPTService extends Service {
    ...
    _messages = [];
    _key = '';
    _key_file_location = `${GLib.get_user_state_dir()}/agsv2/user/ai/${PROVIDERS[this._currentProvider]['key_file']}`;
    _url = GLib.Uri.parse(PROVIDERS[this._currentProvider]['base_url'], GLib.UriFlags.NONE);

    _decoder = new TextDecoder();

    _initChecks() {
        this._key_file_location = `${GLib.get_user_state_dir()}/agsv2/user/ai/${PROVIDERS[this._currentProvider]['key_file']}`;
