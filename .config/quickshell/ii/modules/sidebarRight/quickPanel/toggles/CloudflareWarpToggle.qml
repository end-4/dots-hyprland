...
QuickToggle {
    id: root
    toggled: false
    // don't if work or not
    visible: Quickshell.execDetached(["command", "-v", "warp-cli"])?.length > 0
    customIcon: true
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                if (warpCliCollector.text.length > 0) {
                    root.visible = true // Set visible to true if warp-cli is found
                    fetchActiveState.running = true // Proceed to check status
                } else {
                    root.visible = false // Hide if warp-cli is not installed
                }
            }
        }
    }
